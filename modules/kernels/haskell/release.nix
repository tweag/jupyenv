# fork of https://github.com/IHaskell/IHaskell/blob/575059d33789d400b9677070cdb05a65553379b7/nix/release.nix
{
  ihaskell,
  lib,
  buildEnv,
  ...
  ihaskellOverlay = callPackage "${ihaskell}/nix/ihaskell-overlay.nix" {inherit enableHlint;};
  ...
  in
    runCommand "ihaskell-kernel" {} ''
      export kerneldir=$out/kernels/${name}
      mkdir -p $kerneldir
      cp ${ihaskell + /html}/* $kerneldir
  ...
  jupyterDirLabExtensions = runCommand "ihaskell-labextension" {} ''
    ...
    ln -s ${ihaskell + /jupyterlab-ihaskell/labextension} $out/labextensions/jupyterlab-ihaskell
  '';
